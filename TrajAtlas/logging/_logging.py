...
class _RootLogger(logging.RootLogger):
    ...
    def log(
        ...
        *,
        ...
        from TrajAtlas import settings

        # this will correctly initialize the handles if doing
        # just from TrajAtlas import logging
...
def print_versions():
    ...
    modules = ["TrajAtlas"] + _DEPENDENCIES_NUMERICS + _DEPENDENCIES_PLOTTING
    print(
...
def print_version_and_date():
    ...
    """
    from TrajAtlas import __version__, settings

    print(
        f"Running TrajAtlas {__version__}, on {datetime.datetime.now():%Y-%m-%d %H:%M}.",
        file=settings.logfile,
    )
